...
let expName = 'cgeRDMdraftET';  // from the Builder filename that created this script
let expInfo = {
    'participant': '',
};
...
if (doET) {
    ...
    et.sendCommand(`file_sample_data = ${file_sample_flags}`);
    et.sendCommand("sample_rate 500");
...
flowScheduler.add(etStopRecordingRoutineBegin());
flowScheduler.add(etStopRecordingRoutineEachFrame());
flowScheduler.add(etStopRecordingRoutineEnd());
...
async function experimentInit() {
  ...
  etSetupInstText = new visual.TextStim({
    ...
    languageStyle: 'LTR',
    color: new util.Color([-0.0667,0.6392,1]),  opacity: undefined,
    depth: 0.0 
  });
  ...
  etStopRecordingClock = new util.Clock();
...
function etCameraSetupRoutineBegin(snapshot) {
  return async function () {
    ...
    etSetupInstResp.keys = undefined;
    ...
    for (const thisComponent of etCameraSetupComponents)
...
function etCameraSetupRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && etSetupInstText.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      etSetupInstText.tStart = t;  // (not accounting for frame time here)
    ...
    if (t >= 0.0 && etSetupInstResp.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      etSetupInstResp.tStart = t;  // (not accounting for frame time here)
      etSetupInstResp.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { etSetupInstResp.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { etSetupInstResp.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { etSetupInstResp.clearEvents(); });
    }
    ...
    if (etSetupInstResp.status === PsychoJS.Status.STARTED) {
      ...
      if (_etSetupInstResp_allKeys.length > 0) {
        ...
        etSetupInstResp.rt = _etSetupInstResp_allKeys[_etSetupInstResp_allKeys.length - 1].rt;
...
function etCameraSetupRoutineEnd(snapshot) {
  return async function () {
    ...
    if (currentLoop instanceof MultiStairHandler) {
      currentLoop.addResponse(etSetupInstResp.corr, level);
    }
    psychoJS.experiment.addData('etSetupInstResp.keys', etSetupInstResp.keys);
    if (typeof etSetupInstResp.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('etSetupInstResp.rt', etSetupInstResp.rt);
        routineTimer.reset();
        }
...
function etStartRecordingRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of etStartRecordingComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
var etStopRecordingComponents;
function etStopRecordingRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'etStopRecording' ---
    t = 0;
    etStopRecordingClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    // keep track of which components have finished
    ...
    for (const thisComponent of etStopRecordingComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function etStopRecordingRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'etStopRecording' ---
    // get current time
    t = etStopRecordingClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    // check for quit (typically the Esc key)
    if (psychoJS.experiment.experimentEnded || psychoJS.eventManager.getKeys({keyList:['escape']}).length > 0) {
...
function etStopRecordingRoutineEnd(snapshot) {
  return async function () {
