...
while continueRoutine:
    ...
    if etSetupInstText.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > etSetupInstText.tStartRefresh + 0.0-frameTolerance:
            # keep track of stop time/frame for later
            etSetupInstText.tStop = t  # not accounting for scr refresh
            etSetupInstText.frameNStop = frameN  # exact frame index
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'etSetupInstText.stopped')
