...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\jvonm\\Documents\\GitHub\\cgt\\PyschopyTask\\CGTgamblingSpanTasksAR.py',
...
choices = []
outcomes = []
noRespTxtReal_3 = visual.TextStim(win=win, name='noRespTxtReal_3',
    text='You did not respond in time\n',
    font='Arial',
    pos=[0,0], height=0.08, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=-1.0);
ocGambleReal_3 = visual.Rect(
    win=win, name='ocGambleReal_3',
    width=(0.5, 0.5)[0], height=(0.5, 0.5)[1],
    ori=0, pos=[0,0], anchor='center',
    lineWidth=1,     colorSpace='rgb',  lineColor=[1,1,1], fillColor=[1,1,1],
    opacity=1, depth=-2.0, interpolate=True)
ocSafeReal_3 = visual.Rect(
    win=win, name='ocSafeReal_3',
    width=(0.5, 0.5)[0], height=(0.5, 0.5)[1],
    ori=0, pos=[0,0], anchor='center',
    lineWidth=1,     colorSpace='rgb',  lineColor=[1,1,1], fillColor=[1,1,1],
    opacity=1, depth=-3.0, interpolate=True)
outcomeTextReal_3 = visual.TextStim(win=win, name='outcomeTextReal_3',
    text='',
    font='Arial',
    pos=[0,0], height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=-4.0);
hideGambleReal_3 = visual.Rect(
    win=win, name='hideGambleReal_3',
    width=(0.6, 0.3)[0], height=(0.6, 0.3)[1],
    ori=0, pos=[0,0], anchor='center',
    lineWidth=1,     colorSpace='rgb',  lineColor=[-1,-1,-1], fillColor=[-1,-1,-1],
    opacity=1, depth=-5.0, interpolate=True)
...
Reminder = visual.TextStim(win=win, name='Reminder',
    ...
    languageStyle='LTR',
    depth=0.0);
continealong = keyboard.Keyboard()
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        # *vLeft* updates
        if vLeft.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if vLeft.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > vLeft.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # *nRight* updates
        if nRight.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if nRight.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > nRight.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if choice1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > choice1.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
    ...
    while continueRoutine:
        ...
        if itiText.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > itiText.tStartRefresh + iti + extraITI-frameTolerance:
                # keep track of stop time/frame for later
...
for thisStaticRDM in staticRDM:
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        # *vLeftReal* updates
        if vLeftReal.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if vLeftReal.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > vLeftReal.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # *nRightReal* updates
        if nRightReal.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if nRightReal.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > nRightReal.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # *realChoice* updates
        waitOnFlip = False
        if realChoice.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if realChoice.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > realChoice.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
    ...
    while continueRoutine:
        ...
        if itiTextReal.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > itiTextReal.tStartRefresh + actualITI-frameTolerance:
                # keep track of stop time/frame for later
...
while continueRoutine:
    ...
    if key_resp.status == STARTED and not waitOnFlip:
        theseKeys = key_resp.getKeys(keyList=['v', 'n'], waitRelease=False)
...
# set up handler to look after randomisation of conditions etc
dynamicRDM = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
...
for thisDynamicRDM in dynamicRDM:
    ...
    outcomeTextReal_3.setColor('black', colorSpace='rgb')
    outcomeTextReal_3.setPos(ocLoc)
    outcomeTextReal_3.setText(feedbackRounded)
    hideGambleReal_3.setPos(hideGamLoc)
    # keep track of which components have finished
    showOutcomeDynamicComponents = [noRespTxtReal_3, ocGambleReal_3, ocSafeReal_3, outcomeTextReal_3, hideGambleReal_3]
    for thisComponent in showOutcomeDynamicComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "showOutcomeDynamic" ---
    while continueRoutine and routineTimer.getTime() < 1.0:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        if noRespTxtReal_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            noRespTxtReal_3.frameNStart = frameN  # exact frame index
            noRespTxtReal_3.tStart = t  # local t and not account for scr refresh
            noRespTxtReal_3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(noRespTxtReal_3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'noRespTxtReal_3.started')
            noRespTxtReal_3.setAutoDraw(True)
        if noRespTxtReal_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > noRespTxtReal_3.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                noRespTxtReal_3.tStop = t  # not accounting for scr refresh
                noRespTxtReal_3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'noRespTxtReal_3.stopped')
                noRespTxtReal_3.setAutoDraw(False)
        
        # *ocGambleReal_3* updates
        if ocGambleReal_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ocGambleReal_3.frameNStart = frameN  # exact frame index
            ocGambleReal_3.tStart = t  # local t and not account for scr refresh
            ocGambleReal_3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(ocGambleReal_3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'ocGambleReal_3.started')
            ocGambleReal_3.setAutoDraw(True)
        if ocGambleReal_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > ocGambleReal_3.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                ocGambleReal_3.tStop = t  # not accounting for scr refresh
                ocGambleReal_3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'ocGambleReal_3.stopped')
                ocGambleReal_3.setAutoDraw(False)
        
        # *ocSafeReal_3* updates
        if ocSafeReal_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ocSafeReal_3.frameNStart = frameN  # exact frame index
            ocSafeReal_3.tStart = t  # local t and not account for scr refresh
            ocSafeReal_3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(ocSafeReal_3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'ocSafeReal_3.started')
            ocSafeReal_3.setAutoDraw(True)
        if ocSafeReal_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > ocSafeReal_3.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                ocSafeReal_3.tStop = t  # not accounting for scr refresh
                ocSafeReal_3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'ocSafeReal_3.stopped')
                ocSafeReal_3.setAutoDraw(False)
        
        # *outcomeTextReal_3* updates
        if outcomeTextReal_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            outcomeTextReal_3.frameNStart = frameN  # exact frame index
            outcomeTextReal_3.tStart = t  # local t and not account for scr refresh
            outcomeTextReal_3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(outcomeTextReal_3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'outcomeTextReal_3.started')
            outcomeTextReal_3.setAutoDraw(True)
        if outcomeTextReal_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > outcomeTextReal_3.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                outcomeTextReal_3.tStop = t  # not accounting for scr refresh
                outcomeTextReal_3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'outcomeTextReal_3.stopped')
                outcomeTextReal_3.setAutoDraw(False)
        
        # *hideGambleReal_3* updates
        if hideGambleReal_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            hideGambleReal_3.frameNStart = frameN  # exact frame index
            hideGambleReal_3.tStart = t  # local t and not account for scr refresh
            hideGambleReal_3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(hideGambleReal_3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'hideGambleReal_3.started')
            hideGambleReal_3.setAutoDraw(True)
        if hideGambleReal_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > hideGambleReal_3.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                hideGambleReal_3.tStop = t  # not accounting for scr refresh
                hideGambleReal_3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'hideGambleReal_3.stopped')
...
# keep track of which components have finished
SpanReminderComponents = [Reminder, continealong]
for thisComponent in SpanReminderComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
while continueRoutine:
    ...
    waitOnFlip = False
    if continealong.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        continealong.frameNStart = frameN  # exact frame index
        continealong.tStart = t  # local t and not account for scr refresh
        continealong.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(continealong, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'continealong.started')
        continealong.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(continealong.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(continealong.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if continealong.status == STARTED and not waitOnFlip:
        theseKeys = continealong.getKeys(keyList=['return'], waitRelease=False)
        _continealong_allKeys.extend(theseKeys)
        if len(_continealong_allKeys):
            continealong.keys = _continealong_allKeys[-1].name  # just the last key pressed
            continealong.rt = _continealong_allKeys[-1].rt
            # a response ends the routine
            continueRoutine = False
...
for thisComponent in SpanReminderComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# check responses
if continealong.keys in ['', [], None]:  # No response was made
    continealong.keys = None
thisExp.addData('continealong.keys',continealong.keys)
if continealong.keys != None:  # we had a response
    thisExp.addData('continealong.rt', continealong.rt)
thisExp.nextEntry()
